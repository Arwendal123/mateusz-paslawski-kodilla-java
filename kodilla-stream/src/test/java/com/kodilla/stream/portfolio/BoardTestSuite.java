...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTaskV1() {
        ...
        long QuantityOfTasksInProgress = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(t1 -> t1.getTasks().stream())
                .count();
        double AvgDaysPerTask = sumOfDays / QuantityOfTasksInProgress;

        //Then
        //task2=20, task4=10 task5=0 Avg=(20+10+0)/3=10
        Assert.assertEquals(10.0, AvgDaysPerTask, 0);
    }
    ...
    public void testAddTaskListAverageWorkingOnTaskV2() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        double AvgDaysPerTask = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
        ...
        //Then
        Assert.assertEquals(10, AvgDaysPerTask, 0);
    }
}